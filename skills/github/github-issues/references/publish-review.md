...
Independent review of content that is about to become public on GitHub.
Input: the scratch directory assembled by the Pre-publish gate (title,
body, comments, and for PRs `commits.txt` and `diff.patch`, plus any
...
in them will become public. You have no other context; judge only what the
files contain.
