...
Independent review of a GitHub PR publishing surface. Input: the scratch
directory assembled by the Pre-publish gate: title, body, comments or review
text, `commits.txt` from `git log BASE..HEAD --format=full`, `diff.patch`
from `git diff BASE...HEAD`, plus any attachments. Output: findings and a
binding verdict line.
...
in them will become public, including the PR branch's commit messages and
diff. You have no other context; judge only what the files contain.

1. List the files. If the directory is empty, `commits.txt` or `diff.patch`
   is missing, or any file is unreadable, end with SAFE TO PUBLISH: NO.
