...
Independent review of a GitLab MR publishing surface. Input: the scratch
directory assembled by the Pre-publish gate: title, body, comments or review
text, `commits.txt` from `git log TARGET..SOURCE --format=full`,
`diff.patch` from `git diff TARGET...SOURCE`, plus any attachments. Output:
findings and a binding verdict line.
...
public project, the entire internet — including the MR branch's commit
