...
rules, commit-message rules, versioning and release policy, CI
validation, generated project-level skills). A meta skill's value
...
| Skill | Description |
|---|---|
| [gitlab-release-conventions](gitlab-release-conventions/) | Author a project's release conventions: SemVer policy mapped to commit types, a tokenless tag-check CI job, changelog_config.yml keyed to Changelog trailers, a milestone-per-release policy, and a generated project-level release skill with the gate-before-create flow — with an AGENTS.md section as the fallback deliverable. |
